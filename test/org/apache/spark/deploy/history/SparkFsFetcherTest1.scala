...
class SparkFsFetcherTest1 extends FunSpec with Matchers with MockitoSugar {
  import SparkFsFetcherTest1._
...
object SparkFsFetcherTest1 {
  def main(args: Array[String]) {
    ...
    for (i ← 1 until todos.size()) {
      val analyticJob = todos.get(i)
      val analysisName: String = String.format("%s %s", analyticJob.getAppType.getName, analyticJob.getAppId)
      val analysisStartTimeMillis: Long = System.currentTimeMillis
      val result: AppResult = analyticJob.getAnalysis
      result.save
      val processingTime: Long = System.currentTimeMillis - analysisStartTimeMillis
